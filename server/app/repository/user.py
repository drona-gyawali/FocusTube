...
class UserRepository:
    ...
    async def get_uploaded_links(self, user_id: int) -> List[UploadedLinks]:
        try:
            result = await self.db.execute(select(UploadedLinks).where(UploadedLinks.user_id == user_id))
        except Exception as e:
            logger.error(f"ERROR: {e}")
    ...
    async def update_profile_image(self, user_id: int, image_url: str) -> User | None:
        try:
            ...
            if not user:
                logger.error(f"User with id {user_id} not found for profile image update.")
                return None
